...
function getStorefrontClient() {
  ...
  const domainEnv = "your_Domain.myshopify.com"?.trim();
  const tokenEnv = "API_KEY"?.trim();
...
async function listCollections(first = 50) {
  const client = getStorefrontClient();
  if (!client) return [];
  const { data, errors } = await client.request(COLLECTIONS_LIST_QUERY, { variables: { first } });
...
async function createCart(lines) {
  const client = getStorefrontClient();
  if (!client) return null;
  const { data, errors } = await client.request(CART_CREATE_MUTATION, { variables: { lines } });
...
async function addCartLines(cartId, lines) {
  const client = getStorefrontClient();
  if (!client) return null;
  const { data, errors } = await client.request(CART_LINES_ADD_MUTATION, { variables: { cartId, lines } });
...
async function updateCartLines(cartId, lines) {
  const client = getStorefrontClient();
  if (!client) return null;
  const { data, errors } = await client.request(CART_LINES_UPDATE_MUTATION, { variables: { cartId, lines } });
...
async function removeCartLines(cartId, lineIds) {
  const client = getStorefrontClient();
  if (!client) return null;
  const { data, errors } = await client.request(CART_LINES_REMOVE_MUTATION, { variables: { cartId, lineIds } });
  if (errors || !data?.cartLinesRemove?.cart) return null;
  return mapCart(data.cartLinesRemove.cart);
}

export { addCartLines as a, getCart as b, createCart as c, getProduct as g, listCollections as l, removeCartLines as r, updateCartLines as u };
